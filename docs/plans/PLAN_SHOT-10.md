...
The last review round also found that quotation preparation edits lived only in component
state: navigation discarded payment terms, validity and policy selections without warning,
unlike every other editing surface. Commit `888223c` reports preparation dirty state into the
project page's single `UnsavedChangesGuard` (a second `useBlocker` would shadow the
page-level blocker, since react-router consults only the last registered one) and makes the
