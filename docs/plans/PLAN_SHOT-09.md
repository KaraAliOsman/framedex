...
- **Estado actual:** ✅ `FINAL PROVEN` — SHOT-09 cerrado. Main de cierre final `d94d7c241ee0b6def18ca2b42ea8d1f13f12979f` (merge del hotfix correctivo [PR #32](https://github.com/KaraAliOsman/framedex/pull/32)); tag `shot-09` apunta a `d94d7c2`. Cierre documental previo `b7ccc6c548d3c0455e745d74890fea0acc2abd5b` ([PR #30](https://github.com/KaraAliOsman/framedex/pull/30)); CI final de `main` run `35396326374` — 4/4 success.
- **Hotfix final post-cierre:** `FINAL PROVEN` — merge [PR #32](https://github.com/KaraAliOsman/framedex/pull/32) en `main @ d94d7c241ee0b6def18ca2b42ea8d1f13f12979f` desde head probado `3aafccb`. Dos defectos concretos corregidos: (1) la API exigía `eligible_requirement_keys` pre-ordenados por el cliente; la validación de transporte ahora exige unicidad y el servicio canoniza a representación ordenada antes de hash/persistencia (orden arbitrario aceptado, duplicados/vacíos/malformados rechazados); (2) `generate_artifact()` resolvía el slot idempotente antes de validar autoridad de revisión/orden; ahora valida `project_version_id`+`org_id` activo y `order_id`+`project_version_id`+`org_id` antes de reusar el slot, y la consulta del slot liga `org_id`, `project_version_id` y `artifact_scope`. Regresiones: canonicalización arbitraria servicio+API, duplicados rechazados, aislamiento multi-org con usuario multi-membresía, mismatch orden↔versión rechazado en replay, idempotencia 201→200 preservada. Gauntlet canónico exit 0 en `3aafccb`.
- **Shot:** SHOT-09.
