# DEKOPEN — BIBLIA DE EJECUCIÓN Y SUITE MAESTRA COMPLETA (v1.1.2)
**Versión Oficial:** 1.1.2 (Congelada y Bloqueada tras Micro-Parche Final)
**Hash de Integridad Normativa:** [HASH-RECALCULAR-AL-EMITIR]
**Fecha de Emisión:** 30 de Agosto de 2026
**Compilación:** Documento Único Integral para IA Constructora con hardware_kits, H1-H3, Apéndice A, Apéndice B y Plan SHOT-01..24
...
# DEKOPEN — CONSTITUCIÓN DEL BUILDER (v1.1.1)
**Estado:** Bloqueado / Normativo  
**Vigencia:** Permanente para todo shot de desarrollo, PR y ejecución de agente.

---

## 1. Reglas Fundamentales de Integridad de Datos

1. **NÚMEROS Y FÓRMULAS:** Si un número aparece en un documento de salida, salió estrictamente de `/engine` o de un campo editado manualmente por un usuario humano. **JAMÁS** del texto libre o alucinación de un LLM.
2. **PUREZA DEL ENGINE:** La carpeta `/engine` es un paquete Python puro, sin dependencias de I/O de red, sin Django, sin base de datos y sin llamadas HTTP. Debe ser testeable al 100% de forma aislada mediante `pytest engine/`.
3. **PRECISIÓN DECIMAL:** Prohibido el uso de tipos de coma flotante (`float`) para cualquier dimensión en milímetros (`mm`), cantidades monetarias, porcentajes o coeficientes. Se utilizará `Decimal` con redondeos explícitos (`ROUND_HALF_UP`). En Pesos Chilenos (CLP) el dinero se expresa sin decimales (`Decimal('0')`), en USD con 2 decimales (`Decimal('0.01')`), y en dimensiones milimétricas a 2 decimales para corte (`Decimal('0.01')`).
4. **AISLAMIENTO MULTI-TENANT (RLS):** Toda tabla de negocio en PostgreSQL debe incluir obligatoriamente la columna `org_id` vinculada a `tenancy_organizations`, contar con Row Level Security (RLS) activo y políticas de aislamiento estrictas para `SELECT`, `INSERT`, `UPDATE` y `DELETE`. Un tenant jamás puede leer o inferir precios, series o proyectos de otro. Los catálogos globales (`is_global = TRUE`, `org_id = NULL`) son legibles por todos los usuarios autenticados pero inmutables. Fuga de datos entre organizaciones = Incidente P0.

---

## 2. Reglas de Inteligencia Artificial y Auditoría

5. **AUDITORÍA DE IA PREVIA:** Cualquier sugerencia o acción de escritura propuesta por un modelo de lenguaje (LLM) debe registrarse en la tabla `ai_audit_logs` con su payload de entrada, salida, hash de estado previo, modelo usado, versión de prompt, retención mínima de 24 meses y costo en créditos **ANTES** de aplicar el diff en la base de datos o en el estado de la aplicación.
6. **PROHIBICIÓN DE HARDCODING DE SERIES:** Los parámetros técnicos, holguras, descuentos de vidrio, mermas y dimensiones de perfiles viven en la base de datos (`profile_systems` y tablas asociadas) derivados de fichas técnicas oficiales. Prohibido hardcodear constantes de perfiles en el frontend o backend. Cambiar una fórmula requiere un PR con su respectivo test unitario de caso de oro.
7. **CASOS DE ORO (GOLD CASES G1–G12 + G-Pro1):** Los casos de oro son la verdad matemática inmutable (G1–G12 excepto G10 en Fase 1.5; G-Pro1 cuando exista con sign-off físico). Ningún PR, merge o despliegue puede completarse si cualquiera de los tests de oro arroja una discrepancia superior a `0.00 mm`.
8. **EVOLUCIÓN TÉCNICA FORMAL:** Un cambio en la lógica de cálculo o tolerancia es un cambio de código respaldado por un nuevo caso de oro en `/engine/gold_cases/`, nunca un ajuste de prompt para el modelo de lenguaje.

---

## 3. Arquitectura y Experiencia de Usuario

9. **MONOLITO MODULAR:** Backend estructurado exclusivamente como monolito modular en Django con aplicaciones por dominio (`tenancy`, `catalogs`, `projects`, `pricing`, `documents`, `orders`, `ai_gateway`, `billing`). Queda terminantemente prohibida la creación de microservicios.
10. **LENGUAJE DE TALLER EN ERRORES:** Todo mensaje de error originado por el inspector técnico debe estar redactado en vocabulario estándar de taller de carpintería de PVC en español (ES-CL), explicando claramente: qué falla, por qué falla y ofreciendo un botón de resolución en 1 clic. Prohibido mostrar trazas de error, excepciones no controladas o IDs de base de datos al usuario final.
11. **COMPROMISO HUMANO EXPLÍCITO:** Las acciones críticas (enviar cotización formal al cliente, emitir orden de trabajo al taller, emitir orden de compra a proveedores) requieren interacción humana intencional mediante confirmación en dos pasos. Ningún proceso automático o agente puede ejecutar estas acciones en nombre del usuario sin su autorización explícita.
12. **INMUTABILIDAD DE VERSIONES EMITIDAS:** La tabla `project_versions` congela el snapshot JSON completo del proyecto, lista de materiales (BOM) y precios en cada emisión de documento. Modificar una cotización ya enviada genera automáticamente una nueva revisión (`REV-B`, `REV-C`), conservando intacta la versión previa para auditoría comercial y técnica.
...
17. **DATOS TÉRMICOS Y ACÚSTICOS:** Prohibido calcular o mostrar valores de transmitancia térmica ($U_w$) o atenuación acústica ($R_w$) inventados o extrapolados linealmente sin respaldo de ensayo de laboratorio o ficha certificada del fabricante. Si no existe ficha certificada, el campo se muestra como `"No ensayado"`.
18. **CONTENCIÓN DE MÓDULOS FUTUROS:** El esquema de base de datos para inventario de retazos (`offcut_inventory`) existe en migraciones base, pero su consumo en la optimización de corte en producción queda restringido hasta la Fase 4.
19. **DEFINITION OF DONE (DoD) POR SHOT:** Cada PR o entrega de módulo se considera terminada únicamente cuando se cumplen en su totalidad los siguientes criterios:
    - `pytest engine/` 100% aprobado (Casos de Oro G1–G12 excepto G10 en Fase 1.5).
    - `pytest backend/` 100% aprobado con tests de aislamiento multi-tenant RLS (incluyendo DDL de billing y catálogos globales).
    - `npx vitest run` 100% aprobado en frontend.
    - `ruff check .` sin advertencias ni errores.
    - `mypy engine/` con tipado estricto al 100%.
20. **MANEJO DE AMBIGÜEDAD:** Si durante la implementación se detecta un vacío de especificación en el PRD correspondiente, el agente o desarrollador debe **DETENERSE INMEDIATAMENTE** y solicitar clarificación formal mediante la inserción de `[PENDIENTE-DECISIÓN]` con una nota de una línea, en lugar de asumir comportamientos o inventar reglas de negocio.
21. **AUDITORÍA DE PRECIOS OBLIGATORIA:** Todo cambio de precio, costo, descuento o margen en una posición, proyecto o lista de costo genera obligatoriamente una fila en la tabla `price_audit_logs` ANTES de que el nuevo valor sea persistido en la base de datos.
...
<!-- INICIO DE UI_UX_DESIGN_SYSTEM.md -->
...
<!-- FIN DE PRD-MAQUETAS.md -->

---



<!-- INICIO DE PLAN_DE_EJECUCION_SHOTS.md -->
...
<!-- FIN DE PLAN_DE_EJECUCION_SHOTS.md -->
