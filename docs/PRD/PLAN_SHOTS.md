...
| **SHOT-03** | 0 · s2–3 | PRD-01 §§2–4, §5.1 y §6 (sólo G1–G4); PRD-04 §2 (árbol); PRD-FRONTEND §1.1 (compatibilidad BOM) | **CERRADO** — Engine núcleo: models, geometry FIXED/TURN/TILT_TURN/MULLION, BOM base | `pytest engine/`: **G1, G2, G3, G4 en 0.00**; G8/G9/G11/G12 en xfail declarado |
| **SHOT-04** | 0 · s3 | PRD-03 §1, PRD-19 §3 | **CERRADO** — Auth + tenancy + API skeleton DRF/JWT/OpenAPI + PostHog base + shell app ADOBE dual | Magic link E2E; TOTP owner; OpenAPI $\rightarrow$ TS client autogenerado en CI; PostHog captura eventos base; shell navegable claro/oscuro |
| **SHOT-05** | 0 · s3 | PRD-04, ADOBE, ANIM | **CERRADO** — Canvas 2D mínimo (fijo + cotas) | Dibuja G1 en pantalla = números del engine (<300 ms); cota editable por teclado; snapping |
| **SHOT-06** | 1 · s4–5 | PRD-01 completo, F1 | **CERRADO**: SLIDING_2L, DOOR_ENTRY, AWNING, `hardware_kits`, peso+fallback, pricing puro. Gauntlet local EXIT CODE 0; [PR protegido](https://github.com/KaraAliOsman/framedex/pull/14). Evidencia en `docs/plans/PLAN_SHOT-06.md`; merge PR #14 | **Core Gate Obligatorio:** G5, G6, G7 pasan $0.00\text{ mm}$ + test CI de `golden_example.json` bit a bit. *(Extended Gate: G8, G9, G11, G12 diferibles a SHOT-06B; G10 en Fase 4)* |
...
| **SHOT-09** | 1 · s7–8 | PRD-06, S19 | **CERRADO** — DOC-01…DOC-07 (WeasyPrint + openpyxl) + Pantalla S19 (Pedidos proveedor) | PDF/Excel/OT/corte/checklist/informe con **BOM hash idéntico entre todos**; storage firmado 3600 s; S19 renderiza lista de compra; merge [PR #23](https://github.com/KaraAliOsman/framedex/pull/23) en `main @ 251f441` |
| **SHOT-10** | 1 · s8–9 | PRD-02, S02–S05, S08, S13, S15; gate semántico de catálogos | Flujo proyectos + versiones + clonación + **catálogos manuales: series/artículos, compatibilidad junquillo–vidrio y kits de herraje** | Autómata de estados; freeze REV-A congela snapshot; editar enviada $\rightarrow$ REV-B; CRUD manual de series, junquillos y kits de herraje funcional para el fundador |
