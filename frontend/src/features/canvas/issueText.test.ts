...
import { ProductIssueSeverityEnum, type ProductIssue } from "../../api/generated/models";
...
function geometryFailure(target: string, reason: string): ProductIssue {
  return {
    ...
    severity: ProductIssueSeverityEnum.warning,
    target,
    params: { reason },
  };
}
...
describe("issueText — human-readable, internals never leak", () => {
  ...
  it("names couplings by ordinal", () => {
    const issue: ProductIssue = {
      ...
      severity: ProductIssueSeverityEnum.error,
